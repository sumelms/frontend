...
import FilterAccordion, { FilterSectionProps } from '../../components/FilterAccordion';
import Section from '../../components/Section';
import CourseClassroomsService, { IFilterSection } from '../../services/CourseClassroomsService';
...
const CourseClassrooms: React.FC = () => {
  ...
  const fetchClassrooms = (): void => {
    CourseClassroomsService.fetchClassrooms(params.course as string).then((response): void => {
      setClassrooms(
        response.map(
          (item) =>
            ({
    ...
    <>
      <Section title="Acessar turmas diponiveis">
        <div className="flex w-full space-x-6">
          {filterSections && (
            <FilterAccordion title="Filtrar Turmas" sections={filterSections as FilterSectionProps[]} />
          )}
          <div className="w-full">
            ...
            <div className="flex flex-wrap h-full gap-x-4">
              {classrooms &&
                classrooms.map(({ subject, info }, key) => <CardSubject key={key} subject={subject} info={info} />)}
            </div>
          </div>
        </div>
      </Section>
